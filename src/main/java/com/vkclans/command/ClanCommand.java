...
import com.vkclans.manager.LevelManager;
import com.vkclans.manager.LogManager;
import com.vkclans.manager.MissionManager;
import com.vkclans.manager.RankingManager;
...
public class ClanCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        // Inicializar padrões se necessário
        if (namePattern == null) {
            namePattern = Pattern.compile(VKClans.getInstance().getConfig().getString("validation.name-regex", "^[a-zA-Z0-9_]{3,16}$"));
            tagPattern = Pattern.compile(VKClans.getInstance().getConfig().getString("validation.tag-regex", "^[a-zA-Z0-9]{2,4}$"));
        }
        
        // Cooldown de comandos
        long now = System.currentTimeMillis();
        Long lastUse = commandCooldowns.get(player.getUniqueId());
        if (lastUse != null && now - lastUse < 1000) {
            player.sendMessage("§cAguarde 1 segundo antes de usar outro comando.");
            return true;
        }
        commandCooldowns.put(player.getUniqueId(), now);
        
        // Verifica mundos permitidos
        if (VKClans.getInstance().getConfig().getBoolean("worlds.enabled", false)) {
            java.util.List<String> allowedWorlds = VKClans.getInstance().getConfig().getStringList("worlds.allowed-worlds");
            if (!allowedWorlds.contains(player.getWorld().getName())) {
                player.sendMessage("§cClãs não são permitidos neste mundo.");
                return true;
            }
        }
        ...
        switch (sub) {
            ...
            case "stats": if (player.hasPermission("vkclans.admin")) handleStats(player); else MessageUtil.send(player, "no-permission"); break;
    ...
    private void handleCreate(Player player, String[] args) {
        ...
        if (args.length == 1) {
            // Abrir GUI interativa
            ClanGUI.openClanCreationMenu(player);
            return;
        }
        
        // Modo antigo com argumentos
        ...
        if (!namePattern.matcher(name).matches()) {
            MessageUtil.send(player, "clan-name-invalid-regex");
            return;
        }
        if (!tagPattern.matcher(tagClean).matches()) {
            MessageUtil.send(player, "clan-tag-invalid-regex");
            return;
        }
    ...
    private void handleAccept(Player player, String[] args) {
        ...
        player.sendTitle("§aConvite Aceito!", "§7Bem-vindo ao clã " + clanName);
    ...
    private void handleStats(Player player) {
        ...
        int clanCount = ClanManager.getInstance().getAllClans().size();
        int onlinePlayers = Bukkit.getOnlinePlayers().size();
        
        player.sendMessage("§6=== Estatísticas do VK_Clans ===");
        player.sendMessage("§eClãs Ativos: §f" + clanCount);
        player.sendMessage("§eJogadores Online: §f" + onlinePlayers);
        player.sendMessage("§eMemória Usada: §f" + usedMemory + "MB / " + totalMemory + "MB");
    }
}
