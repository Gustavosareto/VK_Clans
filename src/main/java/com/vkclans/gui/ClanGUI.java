...
public class ClanGUI {
    ...
    /**
     * Abre o menu de criação de clã
     */
    public static void openClanCreationMenu(Player player) {
        Inventory inv = Bukkit.createInventory(null, 27, "Criar Clã");
        
        // Botões principais
        inv.setItem(2, createItem(Material.PAPER, (short) 0, "§eDefinir Nome do Clã", 
            Arrays.asList("§7Clique para digitar o nome via chat", "§7Nome deve ter 3-16 caracteres")));
        
        inv.setItem(4, createItem(Material.SIGN, (short) 0, "§eDefinir Tag do Clã", 
            Arrays.asList("§7Clique para digitar a tag via chat", "§7Tag deve ter 2-4 caracteres, sem &")));
        
        inv.setItem(6, createItem(Material.WOOL, (short) 0, "§eEscolher Cor da Tag", 
            Arrays.asList("§7Abre menu de cores", "§7Selecione uma cor para a tag")));
        
        // Previews
        ClanCreationListener listener = ClanCreationListener.getInstance();
        String name = listener.getPendingName(player.getUniqueId());
        if (name != null) {
            inv.setItem(11, createItem(Material.PAPER, (short) 0, "§aNome: " + name, 
                Collections.singletonList("§7Definido")));
        }
        
        String tag = listener.getPendingTag(player.getUniqueId());
        String color = listener.getPendingColor(player.getUniqueId());
        if (tag != null && color != null) {
            inv.setItem(13, createItem(Material.SIGN, (short) 0, "§aTag: " + color + tag, 
                Collections.singletonList("§7Definida")));
        }
        
        // Ações
        inv.setItem(22, createItem(Material.EMERALD_BLOCK, (short) 0, "§aConfirmar Criação", 
            Arrays.asList("§7Cria o clã com as configurações", "§7Verifique se tudo está definido")));
        
        inv.setItem(26, createItem(Material.BARRIER, (short) 0, "§cCancelar Criação", 
            Arrays.asList("§7Cancela e limpa tudo", "§7Fecha a GUI")));
     ...
     */
    public static void openColorSelectionMenu(Player player) {
        Inventory inv = Bukkit.createInventory(null, 27, "Escolher Cor da Tag");
        
        // Botões de cores
        inv.setItem(10, createColoredWool("§cVermelho", "&c"));
        inv.setItem(11, createColoredWool("§6Laranja", "&6"));
        inv.setItem(12, createColoredWool("§eAmarelo", "&e"));
        inv.setItem(13, createColoredWool("§aVerde", "&a"));
        inv.setItem(14, createColoredWool("§bAzul", "&b"));
        inv.setItem(15, createColoredWool("§dRosa", "&d"));
        inv.setItem(16, createColoredWool("§fBranco", "&f"));
        
        // Voltar
        inv.setItem(26, createItem(Material.BARRIER, (short) 0, "§cVoltar", 
            Collections.singletonList("§7Volta ao menu de criação")));
        
        player.openInventory(inv);
    }
    
    private static ItemStack createColoredWool(String name, String colorCode) {
        short durability = getWoolColor(colorCode);
        ItemStack item = new ItemStack(Material.WOOL, 1, durability);
        ItemMeta meta = item.getItemMeta();
        meta.setDisplayName(name);
        meta.setLore(Arrays.asList("§7Clique para selecionar", "§7Exemplo: " + colorCode + "TAG"));
        item.setItemMeta(meta);
        return item;
    }
    
    private static short getWoolColor(String colorCode) {
        switch (colorCode) {
            case "&c": return 14; // Red
            case "&6": return 1;  // Orange
            case "&e": return 4;  // Yellow
            case "&a": return 5;  // Green
            case "&b": return 11; // Blue
            case "&d": return 6;  // Pink
